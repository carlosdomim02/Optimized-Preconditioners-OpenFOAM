...
{
    int time = sectionStart + blockDim.x * blockIdx.x + threadIdx.x;

    if (time < sectionEnd)
    {
        int face;
        int cell = upperAddr[time];
        int end = times[time + 1];

        for (face = times[time]; face < end; face++)
        {
...
/*for(int i = 0; i < nFaces; i++)
    ...
    /*for(int i = 0; i < nFaces; i++)
    {
        if(ownerStartAddr[upperAddr[i] + 1] - ownerStartAddr[upperAddr[i]] > 0)
...
int main(int argc, char* argv[]) {
    ...
    int parallelSectionsPrecondition;
    // GPU variables
    int* dev_upperAddr; //timesInitSize
    int* dev_lowerAddr; //sizes[1]
    int* dev_writeMap; //timesPreconditionSize
    int* dev_readMap; //sizes[1] * 2
    int* dev_timesInit; //parallelSectionsInit+1
    int* dev_timesPrecondition; //parallelSectionsPrecondition+1
    ...
    int timesInitSize = timesIndicesInit[parallelSectionsInit];
    int timesPreconditionSize = timesIndicesPrecondition[parallelSectionsPrecondition];
    ...
    // Reserve GPU mem
    cudaMalloc(&dev_upperAddr, timesInitSize * sizeof(int));
    cudaMalloc(&dev_lowerAddr, sizes[1] * sizeof(int));
    cudaMalloc(&dev_writeMap, timesPreconditionSize * sizeof(int));
    cudaMalloc(&dev_readMap, sizes[1] * 2 * sizeof(int));
    cudaMalloc(&dev_timesInit, (timesInitSize + 1) * sizeof(int));
    cudaMalloc(&dev_timesPrecondition, (timesPreconditionSize + 1) * sizeof(int));
    ...
    // Send data to GPU
    cudaMemcpy(dev_upperAddr, newUpperAddr, timesInitSize * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_lowerAddr, newLowerAddr, sizes[1] * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_writeMap, writeMap, timesPreconditionSize * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_readMap, readMap, sizes[1] * 2 * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_timesInit, timesInit, (timesInitSize + 1) * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_timesPrecondition, timesPrecondition, (timesPreconditionSize + 1) * sizeof(int), cudaMemcpyHostToDevice);
            ...
            for (int i = 0; i < REPS; i++) {
                ...
                for (int i = 1; i <= parallelSectionsInit; i++)
                {
                    init2 << <blocksPerGridSection, threadsPerBlock >> >
                        (startSection, endSection, dev_upperInit, dev_upperAddr, dev_lowerAddr, dev_timesInit, dev_reciprocalDiagPtr);
            ...
            for (int i = 0; i < REPS; i++) {
                ...
                for (int i = 1; i <= parallelSectionsPrecondition; i++)
                {
                    precondition2 << <blocksPerGridSection, threadsPerBlock >> >
                        (startSection, endSection, dev_diag, dev_upperPrecondition, dev_writeMap, dev_readMap, dev_timesPrecondition, dev_reciprocalDiagPtr);
    ...
    destroy(sizes, psi, diag, lower, upper, ownerStartAddr, losortStartAddr, initialResidual,
        reciprocalDiagPtr, lowerAddr, upperAddr, losortAddr, readMap, writeMap, newLowerAddr, newUpperAddr,
        newUpperInit, newUpperPrecondition, timesInit, timesIndicesInit, timesPrecondition, timesIndicesPrecondition);
