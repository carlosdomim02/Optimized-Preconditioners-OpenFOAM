...
)
{
    ...
    if (cell < nCells) {
        ...
        // Each thread fill his shared memory position and tid=0 fill the extra position
        ...
        for (index = losortStartAddrShared[threadIdx.x]; index < end; index++)
        {
            ...
            readCell = tex1Dfetch<int>(lowerAddr, index);
...
)
{
    ...
    if (cell < nCells) {
        ...
        {
            readCell = tex1Dfetch<int>(upperAddr, face);
            tempValue += upper[face] * diagPtr[readCell];
        }
        diagPtr[cell] -= reciprocalDiagPtr[cell] * tempValue;
    }
...
int main(int argc, char* argv[]) {
    ...
    //Read matrix values
    binaryRead("./matrixBefore.bin", &sizes, &diag, &upperAddr, &lowerAddr, &upper, &lower, &initialResidual, &psi, &ownerStartAddr, &losortAddr, &losortStartAddr);
    ...
    // Describe texture's resources and obejects
    ...
    // Create texture's objects
    cudaTextureObject_t texLosortAddr = 0;
    cudaTextureObject_t texLowerAddr = 0;
    cudaTextureObject_t texUpperAddr = 0;

    resDesc.res.linear.devPtr = dev_losortAddr;
    resDesc.res.linear.sizeInBytes = sizes[1] * sizeof(int);
    cudaCreateTextureObject(&texLosortAddr, &resDesc, &texDesc, NULL);
    resDesc.res.linear.devPtr = dev_lowerAddr;
    resDesc.res.linear.sizeInBytes = sizes[1] * sizeof(int);
    cudaCreateTextureObject(&texLowerAddr, &resDesc, &texDesc, NULL);
    resDesc.res.linear.devPtr = dev_upperAddr;
    resDesc.res.linear.sizeInBytes = sizes[1] * sizeof(int);
    cudaCreateTextureObject(&texUpperAddr, &resDesc, &texDesc, NULL);
    ...
    for (int k = 8; k <= 1024; k *= 2)
    {
        ...
        for (int j = 0; j < 2; j++)
        {
            ...
            cudaEventRecord(start, 0);
            for (int i = 0; i < REPS; i++) {
                precondition << <blocksPerGrid, threadsPerBlock, (2 * threadsPerBlock + 2) * sizeof(int) >> > (dev_diag, sizes[0], dev_upper,
                    texUpperAddr, texLowerAddr, texLosortAddr, dev_ownerStartAddr, dev_losortStartAddr, dev_initialResidual, dev_reciprocalDiagPtr);
                cudaDeviceSynchronize();
            }
            cudaEventRecord(stop, 0);
            cudaEventSynchronize(stop);
