...
)
{
    ...
    for (int i = 0; i < REPS; i++) {
        if (face < end) {
            double expr = upper[face] * upper[face] / reciprocalDiagPtr[lowerAddr[face]];
...
)
{
    ...
    if (face < end) {
        double expr = reciprocalDiagPtr[writeMap[face]] * upper[face] * diag[readMap[face]];
        atomicAdd(&diag[writeMap[face]], expr);
    }
}
...
)
{
    ...
    newTimesIndices = new int[timesIndicesSize + 1];
    copy(timesIndices, timesIndices + timesIndicesSize, newTimesIndices);
    ...
    {
        if (newTimesIndices[indexTime] == i)
        {
            (*times)[time] = i;
            newTimesIndices[indexTime] = time;
            time++;
            indexTime++;
        }
    ...
    // Reordenar de nuevo aprovechando times 
    newWriteMap = new int[nFaces];

    //#pragma omp parallel for
    for (int i = 0; i < timesIndicesSize; i++)
    {
        int count = fixedTimes[newTimesIndices[i]];
        int max = fixedTimes[newTimesIndices[i + 1]];
        while (count < max)
        {
            for (int j = newTimesIndices[i]; j < newTimesIndices[i + 1]; j++)
            {
                if ((*times)[j] < fixedTimes[j + 1])
                {
                    newWriteMap[count] = (*writeMap)[(*times)[j]];
                    (*times)[j]++;
                    count++;
                }
            }
...
)
{
    ...
    groupBySameWriteMapIndex(nFaces, (*parallelSectionsInit), (*timesIndicesInit), timesInit, newUpperAddr);
    groupBySameWriteMapIndex(nFaces2, (*parallelSectionsPrecondition), (*timesIndicesPrecondition), timesPrecondition, writeMap);
    ...
    cout << "Order dependencies finished" << endl;
}
...
int main() {
    ...
    /*for (int j = 0; j < parallelSectionsInit; j++) {
        ...
        init2 << <1, timesIndicesInit[j + 1] - timesIndicesInit[j] >> > (timesIndicesInit[j], timesIndicesInit[j + 1], 
    ...
    for (int i = 0; i < REPS; i++)
    {
        ...
        for (int j = 0; j < parallelSectionsPrecondition; j++) {
            ...
            precondition2 << <1, timesIndicesPrecondition[j + 1] - timesIndicesPrecondition[j] >> > (dev_diag, timesIndicesPrecondition[j], timesIndicesPrecondition[j + 1],
                                                                     dev_writeMap, dev_readMap, dev_upperPrecondition, dev_reciprocalDiagPtr);
        }
    }
    cudaEventRecord(stop, 0);
    ...
    // Get time in seconds
    cudaEventElapsedTime(&elapsedTime, start, stop);
    printf("Time precondition %.6ef s\n", (elapsedTime / 1000) / REPS);
